...
exports.getTdsPreview = getTdsPreview;
...
async function request(req, res, next) {
    try {
        ...
        const idCard = await MemberIdCard_1.MemberIdCard.findOne({ _id: targetCardId, memberId }).exec();
        if (!idCard) {
            return res.status(403).json({
                success: false,
        ...
        const withdrawal = await (0, withdrawalService_1.requestWithdrawal)(memberId, targetCardId, method || "BANK", parseInt(amountPaise), paymentDetails, idempotencyKey || req.headers["x-idempotency-key"] || null);
        res.status(201).json({
            success: true,
            data: withdrawal
        });
...
async function complete(req, res, next) {
    try {
        ...
        const completed = await (0, withdrawalService_1.completeWithdrawal)(withdrawalId);
        res.json({
            success: true,
            data: completed
        });
...
async function reject(req, res, next) {
    try {
        ...
        const rejected = await (0, withdrawalService_1.rejectWithdrawal)(withdrawalId, reason);
        res.json({
            success: true,
            data: rejected
        });
...
async function getHistory(req, res, next) {
    try {
        const withdrawals = await Withdrawal_1.Withdrawal.find({ memberId: req.member?.id })
            .sort({ requestedAt: -1 })
            .exec();
        res.json({
            success: true,
...
async function getTdsPreview(req, res, next) {
    try {
        ...
        const preview = await (0, withdrawalService_1.previewWithdrawal)(memberId, method || "BANK", parseInt(amountPaise));
        res.json({
            success: true,
            data: preview
        });
