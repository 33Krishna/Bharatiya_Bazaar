...
import { SettlementRun } from "../models/SettlementRun";
...
export const EARLY_SETTLEMENT_FEE_PAISE = 25000; // Rs. 250 in paise
export const SECURITY_DEPOSIT_PAISE = 500000; // Rs. 5,000 in paise
export const WALLET_MIN_BALANCE_PAISE = 50000; // Rs. 500 in paise
 ...
 */
export async function getVolumeDiscountPct(
 ...
 */
export async function calculateSettlementBreakdown(
  sales: any[],
  vendor: any,
  options: {
    isEarly?: boolean;
    periodEnd?: Date;
    adminRatePctOverride?: number | null;
  ...
  const earlyFeeSetting = await adminService.getSetting("EARLY_SETTLEMENT_FEE_PAISE", EARLY_SETTLEMENT_FEE_PAISE, "integer", options);
  ...
  const vMember = await Member.findById(vendor.memberId).session(options.session || null).exec();
  if (vMember) {
...
export async function processWeeklySettlement(
  runDate = new Date(),
  options: { adminRatePctOverride?: number | null; actorId?: string | null } = {}
): Promise<any> {
  ...
  const existingRun = await SettlementRun.findOne({ runDate: normalizedRunDate }).exec();
  ...
  const settlementRun = await SettlementRun.findOneAndUpdate(
  ...
  try {
    const vendors = await Vendor.find({ status: { $in: ["ACTIVE", "VERIFIED"] } }).exec();
    ...
    for (const vendor of vendors) {
      ...
      try {
        ...
        if (sales.length > 0) {
          ...
          const settlementArr = await VendorSettlement.create(
            [
              {
                vendorId: vendor.id,
    ...
    const sessionRelease = await mongoose.startSession();
...
export async function processEarlySettlement(
  ...
  try {
    ...
    const settlementRunArr = await SettlementRun.create(
      [
        {
          runDate: now,
          runType: "EARLY",
    ...
    await AuditLog.create(
      [
        {
          actorId: actorId ? new mongoose.Types.ObjectId(actorId) : vendor.memberId,
 ...
 */
export async function sweepVendorInactivity(currentDate = new Date()): Promise<any> {
  const vendors = await Vendor.find({ status: { $in: ["ACTIVE", "INACTIVE", "FROZEN"] } }).exec();
    ...
    if (diffDays >= closedDaysLimit && vendor.status !== "CLOSED") {
      ...
      try {
        ...
        await VendorReferralBonus.updateMany(
          { referredVendorId: vendor.id, status: "PENDING" },
          { memberId: companyWalletTarget }
        ).session(session).exec();
 ...
 */
export async function checkDepositFreeze(
  vendorId: string | mongoose.Types.ObjectId,
  options: { session?: mongoose.ClientSession } = {}
): Promise<void> {
  ...
  const wallet = (vendor.memberId as any)?.mainWallet;
  ...
  if (walletBalance < WALLET_MIN_BALANCE_PAISE && !vendor.isDepositFrozen) {
    vendor.isDepositFrozen = true;
    if (options.session) vendor.$session(options.session);
    await vendor.save();
 ...
 * Admin Fraud Penalty
 */
export async function penalizeVendor(
  vendorId: string | mongoose.Types.ObjectId,
  penaltyType: string,
  transactionAmountPaise = 0,
  ...
  try {
    ...
    const pendingCommissions = await CommissionEntry.find({
      ...
      idCardId: { $in: await MemberIdCard.find({ memberId: vendor.memberId }).select("_id").session(session) }
    }).session(session).exec();
    ...
    vendor.isDepositFrozen = depositRemaining < SECURITY_DEPOSIT_PAISE;
    await vendor.save({ session });
