...
import { invalidateCache } from "../services/adminService";
import bcrypt from "bcrypt";
import mongoose from "mongoose";

export const COMPANY_WALLET_ID = "000000000000000000000001";
...
export const DEFAULT_SETTINGS = [
  ...
  { key: "COMPANY_WALLET_MEMBER_ID", value: COMPANY_WALLET_ID, description: "System member ID for company reserve and closed streams" }
];

export async function seedSettingsAndSuperAdmin(): Promise<void> {
  // 1. Seed Platform Settings
  for (const s of DEFAULT_SETTINGS) {
    try {
      const existing = await PlatformSetting.findOne({ key: s.key }).exec();
      if (!existing) {
  ...
  const companyWallet = await Member.findOneAndUpdate(
    { _id: new mongoose.Types.ObjectId(COMPANY_WALLET_ID) },
    {
      $setOnInsert: {
        _id: new mongoose.Types.ObjectId(COMPANY_WALLET_ID),
